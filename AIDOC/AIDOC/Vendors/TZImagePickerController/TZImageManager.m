...
- (void)creatCustomAlbumName:(NSString *)albumName finishBlock:(void(^)(id))completion{
    ...
    __block BOOL isHaveAlamGroup = NO;
    //ios8 已下
    [self.assetLibrary enumerateGroupsWithTypes:ALAssetsGroupAlbum usingBlock:^(ALAssetsGroup *group, BOOL *stop) {

        if (group) {
            NSString * groupStr = [group valueForProperty:ALAssetsGroupPropertyName];
            if ([groupStr isEqualToString:albumName]) {
              TZAlbumModel *albumModel = [self modelWithResult:group name:albumName];
                if (completion) {
    ...
    if (!isHaveAlamGroup) {//没有找到对应相册
        [self.assetLibrary addAssetsGroupAlbumWithName:albumName resultBlock:^(ALAssetsGroup *group) {
            TZAlbumModel *albumModel = [weakSelf modelWithResult:group name:albumName];
            if (completion) {
                completion (albumModel);
            }
        } failureBlock:^(NSError *error) {
...
//保存图片到自定义响彻
- (void)saveImage:(UIImage *)image  albumNameStr:(NSString *)albumName completion:(void (^)(NSString *error))errorBlcok{
    ...
    if (iOS8Later) {
        ...
        PHFetchResult<PHAsset *> * getAssets = [PHAsset fetchAssetsWithLocalIdentifiers:@[createdAssetID] options:nil];

        [self saveImageAsset:getAssets albumName:albumName errorBlock:errorBlcok];

    }else{
        [self.assetLibrary writeImageDataToSavedPhotosAlbum: UIImageJPEGRepresentation(image, 0.9)
                                                   metadata:nil completionBlock:^(NSURL *assetURL, NSError *error) {
                                                       ...
                                                       [weakSelf.assetLibrary assetForURL:assetURL resultBlock:^(ALAsset *asset) {
                                                           [weakSelf saveImageAsset:asset albumName:albumName errorBlock:errorBlcok];
...
- (void)saveImageAsset:(id)imageAsset albumName:(NSString *)albumName errorBlock:(void (^)(NSString *))finishBlock{

    __block NSError * saveError = nil;
    [self creatCustomAlbumName:albumName finishBlock:^(id model) {

        if ([model isKindOfClass:[NSString class]]) { //ios8 上
            if (!model ) {
                if (finishBlock) {
                    finishBlock(@"创建相册失败");
                }
            ...
            PHFetchResult *collectionsArr = [PHAssetCollection fetchAssetCollectionsWithLocalIdentifiers:@[model] options:nil];
            for (PHAssetCollection  *collection in collectionsArr) {
                if ([collection.localizedTitle isEqualToString:albumName]) {
                    [[PHPhotoLibrary sharedPhotoLibrary] performChangesAndWait:^{
                        ...
                        [collectionChangeRequest addAssets:imageAsset];
        ...
        } else if ([model isKindOfClass:[TZAlbumModel class]]) {
            TZAlbumModel *assetModle =(TZAlbumModel *)model;
            id album = assetModle.result;
...
#pragma clang diagnostic pop
        ...
        AVPlayerItem *playerItem = [[AVPlayerItem alloc] initWithURL:asset];
        if (completion && playerItem) completion(playerItem,nil);
    }
}


- (void)getVideoURLWithAsset:(id)asset completion:(void (^)(NSURL *fileUrl))completion{
...
- (void)startExportVideoWithVideoAsset:(AVURLAsset *)videoAsset completion:(void (^)(NSString *outputPath))completion {
    ...
    if ([presets containsObject:AVAssetExportPresetMediumQuality]) {//这里可以改压缩大小
