...
- (void)setUp{
    ...
    self.textLabel.frame = CGRectMake(0, 0, labelWidth, 30);
    self.textLabel.center = CGPointMake(self.bounds.size.width / 2, self.bounds.size.height / 2 - 10);
    self.textLabel.font = [UIFont boldSystemFontOfSize:24];
    self.textLabel.textColor = rgbColor(0x019fe0);
...
- (void)setCirque{
    for (NSInteger i = 0; i < _valueArray.count; i++) {
        ZFCirque * cirque = [[ZFCirque alloc] initWithFrame:CGRectMake(0, 0, self.frame.size.width, self.frame.size.height)];
